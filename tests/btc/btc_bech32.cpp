...
uint32_t PolyMod(const data& v) {
  ...
  // GF(32), with an implicit 1 in front. If the input is [v0,v1,v2,v3,v4], that
  // polynomial is v(x) = 1*x^5 + v0*x^4 + v1*x^3 + v2*x^2 + v3*x + v4. The
  // implicit 1 guarantees that [v0,v1,v2,...] has a distinct checksum from
  // [0,v0,v1,v2,...].
  ...
  // such a way that the resulting code is a BCH code, guaranteeing detection of
  // up to 3 errors within a window of 1023 characters. Among the various
  ...
  uint32_t c = 1;
  for (auto v_i : v) {
    // We want to update `c` to correspond to a polynomial with one extra term.
    // If the initial value of `c` consists of the coefficients of c(x) = f(x)
    // mod g(x), we modify it to correspond to c'(x) = (f(x) * x + v_i) mod
    // g(x), where v_i is the next input to process. Simplifying: c'(x) = (f(x)
    // * x + v_i) mod g(x)
    //         ((f(x) mod g(x)) * x + v_i) mod g(x)
    //         (c(x) * x + v_i) mod g(x)
    // If c(x) = c0*x^5 + c1*x^4 + c2*x^3 + c3*x^2 + c4*x + c5, we want to
    // compute c'(x) = (c0*x^5 + c1*x^4 + c2*x^3 + c3*x^2 + c4*x + c5) * x + v_i
    // mod g(x)
    ...
    if (c0 & 4)
      c ^= 0x1ea119fa;  //  {4}k(x) = {15}x^5 + {10}x^4 +  {2}x^3 +  {6}x^2 +
                        //  {15}x + {26}
    if (c0 & 8)
      c ^= 0x3d4233dd;  //  {8}k(x) = {30}x^5 + {20}x^4 +  {4}x^3 + {12}x^2 +
                        //  {30}x + {29}
    if (c0 & 16)
      c ^= 0x2a1462b3;  // {16}k(x) = {21}x^5 +     x^4 +  {8}x^3 + {24}x^2 +
                        // {21}x + {19}
  }
  return c;
}
...
bool VerifyChecksum(const std::string& hrp, const data& values) {
  ...
  // the case that appending a 0 to a valid list of values would result in a new
  // valid list. For that reason, Bech32 requires the resulting checksum to be 1
  // instead.
