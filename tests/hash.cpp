...
#define test_btc_hash160 test_hash<fingera_btc_hash160, 20>
#define test_btc_hash256 test_hash<fingera_btc_hash256, 32>

TEST(btc_hash, custom) {
  test_btc_hash256(
  ...
  for (int i = 0; i <= 32; ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      fingera_btc_hash256(in + 64 * j, 64, out1 + 32 * j);
    }
    fingera_btc_hash256_d64(out2, in, i);
